---
description: 全模块深度审查：5 轮分析→红绿验证+修复→交叉检验→基础设施强化，对抗幻觉
allowed-tools: Read, Glob, Grep, Bash(*), Write, Edit, Agent
---

# /deep — 全模块深度审查

`/review --deep` 的独立命令版本。全模块循环强化：5 轮分析→红绿验证+修复→基础设施。
...
**前 3 轮是只读发现，第 4 轮用红绿测试做机械验证+修复，第 5 轮做影响范围扩展。**
...
- 第 4 轮：按模块拆 opus worktree agent，且不得复用发现该 bug 的 agent（降低确认偏差）
...
|------|-----------|-----------|------|
| R1-R3 分析 | Explore agent | `model: "sonnet"` | 代码扫描是机械性工作，sonnet 快 3-4 倍 |
